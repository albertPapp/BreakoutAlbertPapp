...
export default class Level2 extends Phaser.Scene {
  ...
  create() {
    ...
    this.bricks1 = this.physics.add.staticGroup({
      key: ["gold1", "navy1", "red1", "grey1", "green1", "purple1", "grey1"],
      frameQuantity: 5,
      gridAlign: {
        width: 5,
        height: 8,
    ...
    this.bricks = this.physics.add.staticGroup({
      key: ["gold", "navy", "red", "grey", "green", "purple", "grey"],
      frameQuantity: 5,
      gridAlign: {
        width: 5,
        height: 8,
  ...
  hitBrick1(ball, brick) {
    ...
    if (brick.texture.key === "grey1") {
      this.score += 5;
    } else if (brick.texture.key === "purple1") {
      this.score += 10;
    } else if (brick.texture.key === "green1") {
      this.score += 20;
    } else {
      this.score += 40;
    }
    ...
    } else if (selector > 33 && selector < 37) {
    ...
    } else if (selector > 37 && selector < 42) {
    ...
    } else if (selector > 42 && selector < 50) {
    ...
    } else if (selector > 50 && selector < 70) {
    ...
    } else if (selector > 70 && selector < 75) {
    ...
    } else if (selector > 75 && selector < 95) {
    ...
    } else if (selector > 95 && selector < 100) {
  ...
  gameOver() {
    this.registry.set("score2", this.score);
    this.registry.set("level2Done", false);
  ...
  youWon() {
    this.registry.set("score2", this.score);
    this.registry.set("level2Done", true);
  ...
  update() {
    if (this.cursors.left.isDown) {
      this.paddles.children.each((paddle) => {
        paddle.x = Phaser.Math.Clamp(paddle.x, 52, 748);
